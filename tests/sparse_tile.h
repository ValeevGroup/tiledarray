...
template<typename T, typename TagType = std::tuple<> >
  ...
  public:
    ...
    EigenSparseTile (const EigenSparseTile& other) :
	impl_ (other.impl_) {
    }

    /// Shallow assignment operator; see MyTensor::clone() for deep copy
    EigenSparseTile&
    operator= (const EigenSparseTile& other) {
      impl_ = other.impl_;
      return *this;
    }

    /// makes an uninitialized matrix
    EigenSparseTile (const range_type& r) :
	impl_ (
	    std::make_shared<impl_type> (
		std::make_tuple (matrix_type (r.extent ()[0], r.extent ()[1]),
				 r))) {
	...
	return std::get<0> (*impl_).coeffRef (idx[0] - start[0],
					      idx[1] - start[1]);
      }
	...
	if (impl_) {
	  ...
	  for (size_t k = 0; k < mat.outerSize (); ++k)
	    for (typename matrix_type::InnerIterator it (mat, k); it; ++it) {
	      datavec.push_back (
		  Eigen::Triplet < T > (it.row (), it.col (), it.value ()));
	    }
    ...
    EigenSparseTile
    scale (const numeric_type factor,
...
namespace TiledArray {
  ...
  template<typename T, typename TagType>
    struct UnaryBrait<TiledArray::Tensor<T>,EigenSparseTile<T, TagType>> {

      static EigenSparseTile<T, TagType>
      cast (const TiledArray::Tensor<T>& arg) {
...
namespace madness {
  namespace archive {
	...
	static inline void
	load (const Archive& ar, Eigen::Triplet<T>& obj) {
	  int row, col;
	  T value;
	  ar & row & col & value;
	...
	static inline void
	store (const Archive& ar, const Eigen::Triplet<T>& obj) {
