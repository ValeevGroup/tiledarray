...
struct OuterFixture {
  ...
  static int subt(const int left, const int right) {
    return left - right;
  }
...
BOOST_AUTO_TEST_CASE( outer_kernel )
{
  ...
  TiledArray::math::OuterVectorOpUnwindN::outer( & left.front(), & right.front(),
      & result.front(), right.size(), &OuterFixture::add_subt);

  // Check the result
  for(std::size_t i = 0ul; i < TILEDARRAY_LOOP_UNWIND; ++i) {
    for(std::size_t j = 0ul; j < TILEDARRAY_LOOP_UNWIND; ++j) {
      BOOST_CHECK_EQUAL(result[i * right.size() + j], reference[i * right.size() + j] + left[i] - right[j]);
    }
  }
...
BOOST_AUTO_TEST_CASE( outer )
{
  ...
  // Perform outer
  TiledArray::math::outer(left.size(), right.size(), & left.front(), & right.front(),
...
BOOST_AUTO_TEST_CASE( outer_small_left )
{
  ...
  // Perform outer with sizes that are smaller than TILEDARRAY_LOOP_UNWIND for right.
...
BOOST_AUTO_TEST_CASE( outer_small_right )
{
  ...
  for(std::size_t i = 0ul; i < left.size(); ++i) {
    for(std::size_t j = 0ul; j < n; ++j) {
      BOOST_CHECK_EQUAL(result[i * n + j], reference[i * n + j] + left[i] - right[j]);
    }
  }
...
BOOST_AUTO_TEST_CASE( outer_small_left_right )
{
  ...
  TiledArray::math::outer(m, n, & left.front(), & right.front(),
      & result.front(), &OuterFixture::add_subt);
...
BOOST_AUTO_TEST_CASE( outer_fill_kernel )
{
  ...
  TiledArray::math::OuterVectorOpUnwindN::fill( & left.front(), & right.front(),
      & result.front(), right.size(), &OuterFixture::subt);
...
{
  // Perform outer
  TiledArray::math::outer_fill(left.size(), right.size(), & left.front(), & right.front(),
...
BOOST_AUTO_TEST_CASE( outer_and_fill_kernel )
{
  ...
  // Perform outer with the block kernel.
  TiledArray::math::OuterVectorOpUnwindN::fill( & left.front(), & right.front(),
      & reference.front(), & result.front(), right.size(), &OuterFixture::add_subt);
...
BOOST_AUTO_TEST_CASE( outer_and_fill )
{
  ...
  // Perform outer
  TiledArray::math::outer_fill(left.size(), right.size(), & left.front(), & right.front(),
      & reference.front(), & result.front(), &OuterFixture::add_subt);
