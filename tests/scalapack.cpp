...
struct ScaLAPACKFixture {
  ...
  static double make_ta_reference(TA::Tensor<double>& t, TA::Range const& range) {
...
BOOST_AUTO_TEST_CASE(sca_to_uniform_tiled_array_test) {
  ...
  auto ref_ta = TA::make_array< TA::TArray<double> >( *GlobalFixture::world, 
    trange, ScaLAPACKFixture::make_ta_reference
  );
