...
template <typename T>
void vector_test(TiledArray::World &world, const TiledArray::TiledRange &trange, long repeat);
...
template <typename T>
void
vector_test(TiledArray::World& world, const TiledArray::TiledRange& trange, long repeat) {
  ...
  {  // array lifetime scope
    ...
    stop = madness::wall_time();
    if (world.rank() == 0)
      std::cout << "Scale Sum: " << stop - start << " s\n";
    ...
    stop = madness::wall_time();
    if (world.rank() == 0)
      std::cout << "Multiply: " << stop - start << " s\n";
    ...
    stop = madness::wall_time();
    if (world.rank() == 0)
      std::cout << "Power Sum: " << stop - start << " s\n";
