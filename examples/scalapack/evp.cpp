...
int main(int argc, char** argv) {
  auto& world = TA::initialize(argc, argv);
  {
    ...
    auto tensor = TA::make_array<TA::TArray<double>>( world, trange, 
        make_random_ta );

#if 1
    // Symmetrize
    TA::TArray<double> tensor_symm( world, trange );
    ...
    if( world.size() == 1 ){
      ...
      std::cout << "|Matrix - Tensor| = " << (tensor_eigen - matrix_eigen).norm() << std::endl;
    }
    ...
    auto info = scalapackpp::hereig(
        	    scalapackpp::VectorFlag::Vectors,
        	    blacspp::Triangle::Lower,
        	    N, matrix.local_mat().data(), 1, 1, desc, evals.data(),
	...
	scalapackpp::TransposeFlag::Transpose,
	N, N, 1, -evals[i], evecs.local_mat().data(), 1, i+1, desc,
    ...
    double scalapack_evp_check;
    {
      ...
      MPI_Allreduce(&local_norm, &scalapack_evp_check, 1, MPI_DOUBLE, MPI_SUM, world_comm );
      scalapack_evp_check = std::sqrt( scalapack_evp_check );
    }

    if(!world.rank()) std::cout << "EVP (Matrix) |A - XEX**T| = " << scalapack_evp_check << std::endl;



    world.gop.fence();
    // Convert evecs to TA
    auto evecs_ta = evecs.tensor_from_matrix(trange);
    world.gop.fence();
    ...
    if( world.size() == 1 ){
      ...
      std::cout << "|EVecs (matrix) - EVecs(tensor)| = " << (evecs_matrix_eigen - evecs_tensor_eigen).norm() << std::endl;
    }
    ...
    //// Check EVP with TA
    TA::TArray<double> tmp = TA::foreach(evecs_ta,
      [&](TA::Tensor<double>& result, const TA::Tensor<double>& arg) {
    ...
    if(~world.rank()) std::cout << "EVP (Tensor) |A - XEX**T| = " << err_norm << std::endl;
