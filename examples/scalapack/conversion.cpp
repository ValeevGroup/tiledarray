...
TA::TiledRange gen_trange( size_t N, const std::vector<size_t>& TA_NBs ) {
  ...
  std::vector< TA::TiledRange1 > ranges( 2,
    TA::TiledRange1( t_boundaries.begin(), t_boundaries.end() )
  );
...
int main(int argc, char** argv) {
  ...
  auto& world = TA::initialize(argc, argv);
  {
    ...
    // Uniform Tiling = NB:  MAT -> TA
    {
      ...
      auto ref_ta = TA::make_array< TA::TArray<double> >( world, 
        trange, make_ta_reference
      );
      ...
      if( !world.rank() ) {
        std::cout << "|| REF  ||_2                  = " << ref_norm  << std::endl;
        std::cout << "|| TEST ||_2                  = " << test_norm << std::endl; 
        std::cout << "|| MAT -> TA DIFF (UNIF) ||_2 = " << norm_diff << std::endl;
      }
    }
    ...
    // Uniform Tiling = NB: TA -> MAT
    {
      ...
      if( !world.rank() ) 
        std::cout << "|| TA -> MAT DIFF (UNIF) ||_2 = " << norm_diff << std::endl;
    }

    // Random Tiling:  MAT -> TA
    {
      auto trange = gen_trange( N, { 107, 113, 211, 151 } );
      ...
      if( !world.rank() ) {
        std::cout << "|| MAT -> TA DIFF (RAND) ||_2 = " << norm_diff << std::endl;
      }
    }
    ...
    // Random Tiling: TA -> MAT
    {
      ...
      norm_diff = std::sqrt( norm_diff );
      if( !world.rank() ) 
        std::cout << "|| TA -> MAT DIFF (RAND) ||_2 = " << norm_diff << std::endl;
    }
