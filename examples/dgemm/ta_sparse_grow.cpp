...
int main(int argc, char** argv) {
  ...
  try {
    ...
    for(unsigned int sparsity = 100u; sparsity > 0u; sparsity -= 10u) {
      ...
      for(int i = 0; i < repeat; ++i) {
        ...
        if(flop < 1.0)
          flop = 2.0 * c("m,n").sum();
        if(world.rank() == 0)
          std::cout << "Iteration " << i + 1 << "   time=" << time << " s,   speed="
              << flop / time / 1.0e9 << " GFLOPS,   apparent speed=" << app_flop / time / 1.0e9 << " GFLOPS\n";
      }
      ...
      real_sparsity.push_back(100.0 * double(sparse_block_count) / double(sparse_num_blocks * sparse_num_blocks));
    ...
    if(world.rank() == 0) {
      std::cout << "\n\nResults:\n"
          "sparsity (%), time (s), speed (GFLOPS), apparent speed (GFLOPS)\n";
      for(unsigned int i = 0; i < 10; ++i) {
        std::cout << real_sparsity[i] << ", " << times[i] << ", " << speeds[i] << ", " << app_speeds[i] << "\n";
      }
    }
