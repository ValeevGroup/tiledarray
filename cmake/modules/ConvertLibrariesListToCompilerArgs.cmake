...
macro(convert_libs_to_compargs _args _libs )
  ...
  foreach (_lib ${_libs})
    ...
    if(APPLE AND "${_ext}" STREQUAL ".framework")
      # Store the framework path (if it is not the default system path) and name.
      if(NOT ${_path} STREQUAL "/System/Library/Frameworks")
      ...
      foreach(_prefix ${CMAKE_FIND_LIBRARY_PREFIXES})
        if(_libname MATCHES "^${_prefix}")
          # Strip the library prefix from the name.
          string(REGEX REPLACE "^${_prefix}(.*)$" "\\1" _name ${_libname})
          break()
        endif()
      endforeach()

      # Store the library path and base name
