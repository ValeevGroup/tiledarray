...
namespace TiledArray {
  ...
  template<typename T, unsigned int DIM> class AbstractArray {
    public:
    ...
    public:
      ...
      AbstractArray(const IndexType dim,
                    const ValueType defaultValue = ValueType()) :
        m_dim(dim), m_defaultValue(defaultValue) {
      }
      
      virtual inline const ValueType& At(const IndexType& index) const = 0;

      virtual inline ValueType& At(const IndexType& index) = 0;
      ...
      void Resize(const IndexType& newDim) {
        this->m_dim = newDim;
      }
  ...
  template<typename T, unsigned int DIM> class DenseArray :
      public AbstractArray<T, DIM> {
    ...
    public:
      ...
      DenseArray(const IndexType& dim, const ValueType& val = ValueType()) :
        AbstractArray<T, DIM>(dim, val),
            m_data(VectorOps<IndexType, DIM>::selfProduct(dim), val) {
      }
      
      DenseArray(const IndexType& dim, const ValueType& (*func)(const IndexType&)) :
        AbstractArray<T, DIM>(dim),
            m_data(VectorOps<IndexType, DIM>::selfProduct(dim)) {
        bool atEnd = false;
        for (IndexType ItDim(0); !atEnd; atEnd
            = VectorOps<IndexType, DIM>::increment(ItDim))
          this->At(ItDim) = func(ItDim);
      }
      
      // Accessor
      virtual inline ValueType& At(const IndexType& index) {
        return this->m_data[Offset(index)];
      }
      
      virtual inline const ValueType& At(const IndexType& index) const {
        return this->m_data[Offset(index)];
      }
      
      virtual void Resize(const IndexType& newDim) {
    ...
    private:
      ...
      unsigned int Offset(const IndexType& index) const {
        unsigned int n = 0;
        unsigned int m = 1;
  ...
  template<typename T, unsigned int DIM> class SparceArray :
      public AbstractArray<T, DIM> {
    public:
      typedef typename AbstractArray<T, DIM>::ValueType ValueType;
      typedef typename AbstractArray<T, DIM>::IndexType IndexType;
