...
#ifndef TILEDARRAY_DIST_EVAL_SUMMA_H__INCLUDED
#define TILEDARRAY_DIST_EVAL_SUMMA_H__INCLUDED
...
namespace TiledArray {
  namespace detail {
    ...
    class Summa : public madness::WorldObject<Summa<Left, Right, Op> >, public ContractionEvalImpl<Left, Right, Op> {
    protected:
      typedef madness::WorldObject<Summa<Left, Right, Op> > WorldObject_; ///< Madness world object base class
      typedef ContractionEvalImpl<Left, Right, Op> ContractionEvalImpl_;
      typedef typename ContractionEvalImpl_::DistEvalImpl_ DistEvalImpl_;
    ...
    public:
      ...
      typedef typename ContractionEvalImpl_::size_type size_type; ///< size type
      typedef typename ContractionEvalImpl_::value_type value_type; ///< The result value type
      typedef typename ContractionEvalImpl_::left_type left_type; ///< The left tensor type
      typedef typename ContractionEvalImpl_::left_value_type left_value_type; /// The left tensor value type
      typedef typename ContractionEvalImpl_::right_tensor_type right_type; ///< The right tensor type
      typedef typename ContractionEvalImpl_::right_type right_value_type; ///< The right tensor value type
      typedef typename ContractionEvalImpl_::pmap_interface pmap_interface; ///< The process map interface type
    ...
    private:
          ...
          const std::pair<madness::Future<std::vector<col_datum> >, madness::Future<std::vector<row_datum> > >& col_row_k1)
      {
        ...
        if(assign) {
          ...
          ContractionEvalImpl_::left().release();
          ContractionEvalImpl_::right().release();
        } else {
          if(task_row_col_k2) {
    ...
    public:
      ...
      Summa(const Left& left, const Right& right) :
          ...
          ContractionEvalImpl_(left, right),
          row_group_(),
          col_group_(),
          ...
          for(size_type i = rank_row_; i < m_; i += proc_rows_)
            for(size_type j = rank_col_; j < n_; j += proc_cols_) {
              ...
              results_.push_back(result_datum(ij,
                  reduce_pair_task(get_world(), contract_reduce_op(*this), &counter)));
              DistEvalImpl_::set(ij, results_.back().second.result());
              ++task_count;
            }
...
#endif // TILEDARRAY_DIST_EVAL_SUMMA_H__INCLUDED
