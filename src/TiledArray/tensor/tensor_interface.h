...
namespace detail {
    ...
    class TensorInterface {
       public:
        ...
        typedef typename std::add_lvalue_reference<typename std::add_const<
            T>::type>::type const_reference;  ///< Element reference type
       ...
       public:
        ...
        template <typename U, typename std::enable_if<std::is_convertible<
                                  typename TensorInterface<U, R>::pointer,
        ...
        template <typename T1,
                  typename std::enable_if<detail::is_tensor<T1>::value>::type* =
                      nullptr>
        TensorInterface_& operator=(const T1& other) {
            TA_ASSERT(data_ != other.data());

            detail::inplace_tensor_op(
                [](numeric_type& restrict result,
                   const typename T1::numeric_type arg) { result = arg; },
                *this, other);
        ...
        void reset_data(T* data) { data_ = data; }
        ...
        /// Use a binary, element wise operation to construct a new, permuted
        /// tensor
        ...
        template <
            typename Right, typename Op,
            typename std::enable_if<is_tensor<Right>::value>::type* = nullptr>
        result_tensor binary(const Right& right, Op&& op,
        ...
        /// \throw TiledArray::Exception When the range of this tensor is not
        /// equal
        ...
        /// Use a unary, element wise operation to construct a new, permuted
        /// tensor

        /// \tparam Op The unary operation type
        /// \param op The unary operation
        /// \param perm The permutation to be applied to this tensor
        /// \return A permuted tensor with elements that have been modified by
        /// \c op
        /// \throw TiledArray::Exception When this tensor is empty.
        ...
        /// \return A new tensor where the elements are the sum of the elements
        /// of
        ...
        /// of
        /// \c this are scaled by \c factor
        result_tensor scale(numeric_type factor,
                            const Permutation& perm) const {
            return unary([=](const numeric_type a) { return a * factor; },
                         perm);
        }
        ...
        TensorInterface_& scale_to(numeric_type factor) {
            return inplace_unary(
                [=](numeric_type& restrict res) { res *= factor; });
        }
        ...
        /// of
        /// \c this and \c other
        template <typename Right, typename std::enable_if<
                                      is_tensor<Right>::value>::type* = nullptr>
        result_tensor add(const Right& right) const {
            return binary(
                right,
                [](const numeric_type l,
                   const typename TiledArray::detail::scalar_type<Right>::type
                       r) { return l + r; });
        }
        ...
        /// of
        /// \c this and \c value
        result_tensor add(const numeric_type value,
                          const Permutation& perm) const {
        ...
        TensorInterface_& add_to(const numeric_type value) {
            return inplace_unary(
                [=](numeric_type& restrict res) { res += value; });
        }
        ...
        /// \return A new tensor where the elements are the different between
        /// the
        ...
        template <typename Right, typename std::enable_if<
                                      is_tensor<Right>::value>::type* = nullptr>
        result_tensor subt(const Right& right,
                           const numeric_type factor) const {
            return binary(
                right,
                [=](const numeric_type l,
                    const typename TiledArray::detail::scalar_type<Right>::type
        ...
        /// Scale and subtract this and \c right to construct a new, permuted
        /// tensor
        ...
        /// the
        /// elements of \c this and \c value
        result_tensor subt(const numeric_type value,
                           const Permutation& perm) const {
        ...
        /// \return A new tensor where the elements are the product of the
        /// elements
        ...
        template <typename Right, typename std::enable_if<
                                      is_tensor<Right>::value>::type* = nullptr>
        result_tensor mult(const Right& right,
                           const numeric_type factor) const {
            return binary(
                right,
                [=](const numeric_type l,
        ...
        /// \return A new tensor that contains the negative values of this
        /// tensor
        ...
        /// \return The reduced value
        template <
            typename Right, typename ReduceOp, typename JoinOp,
            typename std::enable_if<is_tensor<Right>::value>::type* = nullptr>
        ...
        numeric_type sum() const {
            auto sum_op = [](numeric_type& restrict res,
        ...
        numeric_type product() const {
            auto mult_op = [](numeric_type& restrict res,
                              const numeric_type arg) { res *= arg; };
        ...
        numeric_type squared_norm() const {
            auto square_op = [](numeric_type& restrict res,
                                const numeric_type arg) { res += arg * arg; };
            auto sum_op = [](numeric_type& restrict res,
                             const numeric_type arg) { res += arg; };
        ...
        /// \return The vector norm of this tensor
        numeric_type norm() const { return std::sqrt(squared_norm()); }
        ...
        numeric_type min() const {
            auto min_op =
                [](numeric_type& restrict res, const numeric_type arg) {
                res = std::min(res, arg);
            };
            return reduce(min_op, min_op,
        ...
        numeric_type abs_min() const {
            ...
            return reduce(abs_min_op, min_op,
                          std::numeric_limits<numeric_type>::max());
        }
        ...
        numeric_type dot(const Right& other) const {
            auto mult_add_op = [](
                numeric_type& res, const numeric_type l,
                const typename TiledArray::detail::scalar_type<Right>::type r) {
                res += l * r;
            };
            auto add_op = [](numeric_type& restrict res,
