...
//      operator T&() const { return get(); }
      ...
      auto range() const -> decltype(tensor_->range()) { return get().range(); }

      /// Tensor data pointer accessor

      /// \return A pointer to the tensor data
      auto data() const -> decltype(get().data()) { return get().data(); }

      /// Check for an empty tensor

      /// \return \c true if the tensor is empty, otherwise \c false.
      auto empty() const -> decltype(get().empty()) { return get().empty(); }
