...
namespace TiledArray {
  namespace detail {
    ...
    /// \tparam Op The transform operator type.
    template <typename Iter, typename Op>
    class UnaryTransformIterator<IndexedIterator<Iter>, Op> {
    private:
      // Give access to other Transform iterator types.
      template <typename, typename>
      friend class UnaryTransformIterator;
    ...
    public:
      ...
      typedef value_type reference;                             ///< Reference type to iterator value
      typedef std::input_iterator_tag iterator_category;        ///< Iterator category type
      typedef UnaryTransformIterator<IndexedIterator<Iter>, Op> this_type;       ///< This object type
      typedef IndexedIterator<Iter> base_iterator;                               ///< The base iterator type
      typedef typename base_iterator::index_type index_type;

      /// Constructor

      /// \param it The base iterator
      /// \param op The transform operator
      UnaryTransformIterator(base_iterator it, Op op = Op()) :
          it_(it), op_(op)
      { }
      ...
      const index_type& index() const { return it_.index(); }
