...
class Tile {
 ...
 public:
  ...
  template <typename Index>
  decltype(auto) block(const Index& lower_bound, const Index& upper_bound) {
    TA_ASSERT(pimpl_);
  ...
  decltype(auto) block(const std::initializer_list<size_type>& lower_bound,
  ...
  template <typename Index>
  decltype(auto) block(const Index& lower_bound,
