...
/// \return The size of c-stype array
template <typename T, std::size_t N>
inline constexpr std::size_t size(const std::array<T, N>&) {
  return N;
}
...
template <typename T, typename = std::enable_if_t<
                          has_member_function_size_anyreturn_v<const T>>>
inline auto size(const T& a) {
  return a.size();
}
...
template <typename T>
inline auto size(std::initializer_list<T> a) {
  return a.size();
}
...
inline auto size(const std::tuple<Ts...>& a) {
...
/// \return A pointer to the first element of the container, \c v
template <typename T, typename = std::enable_if_t<
                          has_member_function_data_anyreturn_v<const T>>>
...
template <typename T>
inline T* data(std::initializer_list<T>& l) {
  return l.begin();
}
...
template <typename T>
inline const T* data(const std::initializer_list<T>& l) {
  return l.begin();
}
