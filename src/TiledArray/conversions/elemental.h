...
El::DistMatrix<typename Array::element_type> matrix_to_el(Array const &A,
                                                          El::Grid const &g) {
...
DistArray<Tensor<T>, DensePolicy> el_to_matrix(
  ...
  TA_ASSERT(trange.rank() == 2);
  TA_USER_ASSERT(uniformly_blocked(trange),
                 "The output TiledRange must be uniformly blocked.");
#if !defined(EL_RELEASE)
  TA_USER_ASSERT(
      madness::ThreadPool::size() == 0,
      "TA::el_to_matrix(): Elemental compiled in Debug mode is not re-entrant,"
      " must execute with MAD_NUM_THREADS=1 (and without TBB). Recompile "
      "Elemental in Release mode to use MAD_NUM_THREADS>1 or to use TBB.");
#endif
  ...
  El::DistMatrix<T, El::MC, El::MR, El::BLOCK> Mb(M.Height(), M.Width(),
                                                  M.Grid(), row_bs, col_bs);
  Mb = M;
...
/// should be combined to make a matrix. Permutations are not currently
/// supported.
...
class TensorFlattening {
 ...
 public:
  ...
  TensorFlattening(std::vector<int> const &left, std::vector<int> const &right)
...
/// @param tf a TensorFlattening object that fuses indices in tensors with more
/// than 2 dimensions
template <typename Array>
El::DistMatrix<typename Array::element_type> array_to_el(
  ...
  if (A.trange().rank() == 2) {
    ...
    TA_USER_ASSERT(
        false,
        "array_to_el currently only supports converting matrices, higher order "
        "tensor conversions will be supported at a latter date.");
  }
}
...
DistArray<Tensor<T>, DensePolicy> el_to_array(
    El::AbstractDistMatrix<T> const &M, World &world, TiledRange const &trange,
    TensorFlattening const &tf = TensorFlattening()) {
  if (trange.rank() == 2) {
    // Flattening is not needed for the matrix case.
    ...
    TA_USER_ASSERT(
        false,
        "el_to_array currently only supports converting matrices, higher order "
        "tensor conversions will be supported at a latter date.");
  }
}
