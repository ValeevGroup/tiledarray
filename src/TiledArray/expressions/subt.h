...
#ifndef TILEDARRAY_EXPRESSIONS_SUBT_H__INCLUDED
#define TILEDARRAY_EXPRESSIONS_SUBT_H__INCLUDED
...
namespace TiledArray {
  namespace expressions {
    ...
    class Subt : public Binary<Subt<Left, Right> > {
    public:
      typedef Binary<Subt<Left, Right> > Binary_; ///< Binary base class type
      ...
      typedef TiledArray::math::Subt<value_type, typename left_type::eval_type,
      ...
      Subt(const left_type& left, const right_type& right) : Binary_(left, right) { }

      /// Copy constructor

      /// \param other The expression to be copied
      Subt(const Subt<Left, Right>& other) : Binary_(other) { }
    ...
    /// Scaled subtraction expression

    /// \tparam Left The left-hand expression type
    /// \tparam Right The right-hand expression type
    template <typename Left, typename Right>
    class ScalSubt : public Binary<ScalSubt<Left, Right> > {
    public:
      typedef Binary<ScalSubt<Left, Right> > Binary_; ///< Binary base class type
      typedef typename Binary_::Base_ Base_; ///< Base expression type
      typedef Left left_type; ///< The left-hand expression type
      typedef Right right_type; ///< The right-hand expression type
      typedef typename left_type::eval_type value_type; ///< The result tile type
      typedef TiledArray::math::ScalSubt<value_type, typename left_type::eval_type,
          typename right_type::eval_type, left_type::consumable,
          right_type::consumable> op_type; ///< The tile operation type
    ...
    public:
      ...
      ScalSubt(const Subt<Left, Right>& arg, const scalar_type factor) :
      ...
      /// \param factor The scaling factor
      ScalSubt(const ScalSubt<Left, Right>& arg, const scalar_type factor) :
      ...
      /// \param other The expression to be copied
      ScalSubt(const ScalSubt<Left, Right>& other) :
...
#endif // TILEDARRAY_EXPRESSIONS_SUBT_H__INCLUDED
