...
namespace TiledArray {
  ...
  template<class T>
  void reverse_permute(std::vector<T>& vec);

  template<unsigned int DIM, class T>
  void permute(std::vector<T>& vec, const Tuple<DIM>& perm);
  ...
  public:
    ...
    inline Tuple<DIM> operator +(const Tuple<DIM>& tup) const {
      Tuple<DIM> ret;
      VectorOps<std::vector<value_t>, DIM>::add(ret.m_values, m_values, tup.m_values);
      return ret;
    }
    ...
    inline Tuple<DIM> operator *(const Tuple<DIM>& other) const {
      Tuple<DIM> ret;
      VectorOps<std::vector<value_t>, DIM>::mult(ret.m_values, m_values, other.m_values);
      return ret;
    }
      
    inline Tuple<DIM> operator /(const Tuple<DIM>& other) const {
      Tuple<DIM> ret;
      VectorOps<std::vector<value_t>, DIM>::div(ret.m_values, m_values, other.m_values);
      return ret;
    }
      
    inline Tuple<DIM> operator %(const Tuple<DIM>& tup) const {
      Tuple<DIM> ret;
      VectorOps<std::vector<value_t>, DIM>::mod(ret.m_values, m_values, tup.m_values);
      return ret;
    }
    ...
    Tuple<DIM>& permute() {
      ::TiledArray::permute(m_values);
      return (*this);
    }

    /// Reverse permutation of set by one.
    Tuple<DIM>& reverse_permute() {
      ::TiledArray::reverse_permute(m_values);
      return *this;
    }
    ...
    Tuple<DIM>& permute(const Tuple<DIM>& perm) {
      ::TiledArray::permute(m_values, perm);
      return (*this);
    }
  ...
  std::ostream& operator<<(std::ostream& output, const Tuple<DIM>& tup) {
    output << "(";
  ...
  * User defined permutation of a tuple.
  *
  * @param vec - vector<T> that will be permuted according to perm. The content of vec will be modified.
  * @param perm - Tuple must include each index number (0, 1, ..., DIM-1) once and only once.
  */
  template<unsigned int DIM, class T>
  void permute(std::vector<T>& vec, const Tuple<DIM>& perm) {
    ...
    for(unsigned int dim = 0; dim < vec.size(); ++dim)
    ...
    const std::vector<T> temp(vec);
    for(unsigned int dim = 0; dim < DIM; ++dim)
      vec[dim] = temp[perm[dim]];
  }
