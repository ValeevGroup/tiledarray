...
namespace TiledArray {
  ...
  template<unsigned int DIM> class Orthotope {
      ...
      void init() {
        ...
        for (int dim = DIM - 1; dim > 0; --dim)
          this->m_linear_step[dim - 1] = (this->m_high[dim] - this->m_low[dim])
              * this->m_linear_step[dim];
      }
    ...
    public:
      ...
      Orthotope(const Range* ranges) :
        m_ranges(ranges, ranges + DIM - 1), m_high(), m_low(), m_size() {
      ...
      Orthotope(const size_t* ranges[DIM], const size_t tiles[DIM]) :
        m_ranges(DIM, Range()), m_high(), m_low(), m_size() {
        for (int dim = 0; dim < DIM; ++dim) {
          assert(ranges[dim]);
      ...
      Orthotope(const std::vector<Range>& ranges) :
        m_ranges(ranges), m_high(), m_low(), m_size() {
      ...
      inline range_iterator begin(const unsigned int dim) const {
        assert(dim < DIM);
        return this->m_ranges[dim].begin();
      }
      
      inline range_iterator end(const unsigned int dim) const {
        assert(dim < DIM);
        return this->m_ranges[dim].end();
      }
      
      inline Range& operator [](const unsigned int dim) {
        assert(dim < DIM);
      ...
      Tuple<DIM> high(const Tuple<DIM>& tile_index) const {
        assert(tile_index < this->count());
        Tuple<DIM> tmp;
        for (int dim = 0; dim < DIM; ++dim)
      ...
      Tuple<DIM> size(const Tuple<DIM>& tile_index) const {
        assert(tile_index < this->count());
        Tuple<DIM> tmp;
        for (int dim = 0; dim < DIM; ++dim)
      ...
      // Number of elements contained in the orthotope.
      inline size_t count(const Tuple<DIM>& tile_index) const {
        return VectorOps<Tuple<DIM>, DIM>::selfProduct(this->high(tile_index)
            - this->low(tile_index));
      }
      
      // Return the tile that contains an element index.
      inline Tuple<DIM> tile(const Tuple<DIM>& element_index) const {
      ...
      inline std::pair<Tuple<DIM>, Tuple<DIM> > tile_range(
                                                         const Tuple<DIM>& tile_index) const {
        assert(tile_index < this->count());
